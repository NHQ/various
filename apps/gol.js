...
function train(game, epochas=argv.e || 10, futuras=argv.f || 3){// flatten the thing or what?  
  ...
  for(var i = 0; i < epochas; i++){
    ...
    for(var b =0; b < batch_size[0]; b++){
      let c = batch.push($.variable({shape: [1, shape[1], shape[2], 1], max: .95, init: 'randomUniform', trainable: false}).layer.round())
    ...
    for(var j = 0; j < futuras; j++){
      optimus.minimize(()=>{
        ...
        let loss = tf.losses.absoluteDifference(target.reshape(batch_size), output)//.mean()
        //console.log(output.dataSync())
        loss.print()
        let reg = regularize(.01, .01)
        //reg.print()
