...
function rnn({input_shape, layers, depth=3, mag=.1, input=undefined, ortho=false}){
  ...
  var flow = layers.reduce((a, e) => {
    ...
    var fb_w = new Array(depth).fill(0).map(e => $.variable({shape: [config.size, config.size], trainable: true}).layer ) 
